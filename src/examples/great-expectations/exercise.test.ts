...
describe('Kanban Board', () => {
  ...
  it('should include an added status in board.statuses using #addStatus', () => {
    ...
    board.addStatus(mockStatus);
    // Verify that the new status is—in fact—now in board.statuses.
    expect(board.statuses).toContain(mockStatus);
  });
  ...
  it('should remove a status using #removeStatus', () => {
    ...
    const mockStatus = 'some-random-status';
    board.addStatus(mockStatus);
    expect(board.statuses).contain(mockStatus);
    board.removeStatus(mockStatus);
    ...
    expect(board.statuses).not.contain(mockStatus);
  });
});
